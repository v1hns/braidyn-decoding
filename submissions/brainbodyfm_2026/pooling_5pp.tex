...
further on all four events (Section~\ref{sec:matched}).
...
$\Delta F/F$ parcellated into 44 Allen Common Coordinate Framework regions for 25 mice, each
completing $\sim$15 daily operant sessions over $\sim$3 weeks. For each of the four events we build a
44-dimensional evoked feature representing the mean $\Delta F/F$ over $0$--$1$\,s minus the baseline,
per parcel, and decode it against matched negatives with $\ell_2$-regularized logistic regression,
scored by the area under the receiver-operating characteristic curve ($\auc$).

Sessions are grouped into an early block and a late block. The personal decoder leaves out one early
session at a time; training is on the mouse's remaining early sessions and scoring is both on the
left-out session and the late block. The pooled decoder trains once on the \emph{other} mice's early
blocks and is scored on the same held-out data.

Aging is the accuracy lost as testing moves from early to late. The per-mouse asymmetry is personal
aging minus pooled aging, positive when the personal decoder ages more. Significance is a paired
sign-flip permutation test across mice, with cluster-bootstrap confidence intervals.

Holding out whole sessions rather than trials is essential, as only the within-subject arm can share
a session between training and test. The external validation cohort is Allen Visual Behavior
two-photon \citep{allenvb2023}: containers spanning at least 14 days whose experiments are all in
VISp within a single 50\,\textmu m depth bin, leaving 23 mice after quality control. Because
different animals have no neuron correspondence, the representation that is comparable is a
permutation-invariant summary of the population response, giving a 27-dimensional feature regardless
of how many cells were segmented.

The decoded contrast is \texttt{is\_change}, a real image change against a matched catch trial with
no change. Appendix~\ref{app:methods} gives both cohorts in full.

\section{Results}

\subsection{A pooled decoder ages more slowly than a personal one}
\label{sec:asym}
...
personal decoder aging more in 73\% of pairs; the per-event breakdown is in
Table~\ref{tab:asym} of Appendix~\ref{app:perevent}, and the two cohorts are summarised together
in Table~\ref{tab:allen}.
...
\label{sec:protocol}
Every number reported above holds out whole recording sessions. Repeating the analysis with a
...
untouched. The inflation therefore lands on one side of the difference, and for lever pull this is
the entire effect: a significant-looking $+0.014$ ($p = 0.001$) becomes $+0.003$ ($p = 0.37$). Any
comparison of a within-subject decoder against a cross-subject one is exposed to this, since only
the within-subject arm can share sessions between train and test.
...
(Appendix~\ref{app:decay}), while the personal decoder stays flat. Here the pooled decoder is flat
...
\textbf{Code and data availability.} Both cohorts are public (DANDI:001425; Allen Brain
Observatory). The analysis code and streaming feature pipeline are released.
...
We measure at the mesoscale, using parcel-averaged activity, and while the two-photon replication
does reach single neurons it does so only at one depth in one visual area.

The analysis covers a two-week interval in one species, and nothing here establishes how the
asymmetry behaves over the months-to-years horizon on which a deployed decoder has to survive.
...
common feature space has to be constructed rather than assumed. The
...
\texttt{is\_change}, a real image change against a matched catch trial with no change, which is a stimulus contrast
