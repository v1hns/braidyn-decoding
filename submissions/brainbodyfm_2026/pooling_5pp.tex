...
completing $\sim$15 daily operant sessions over $\sim$3 weeks. For each onset of four events (lever
pull, lick, reward, tone) we build a 44-dimensional evoked feature -- the mean $\Delta F/F$ over
$0$--$1$\,s minus the $-0.5$--$0$\,s baseline, per parcel -- and decode it against matched negatives
with $\ell_2$-regularized logistic regression, scored by area under the receiver-operating
...
sessions and scoring both on the held-out session and on the late block; the pooled decoder trains
once on the other mice's early blocks and is scored on the same held-out data. Aging is the accuracy
lost as testing moves from early to late with training fixed, and the per-mouse asymmetry is personal
aging minus pooled aging, positive when the personal decoder ages more. Significance is a paired
sign-flip permutation test across mice (20{,}000 permutations), with cluster-bootstrap confidence
intervals. Holding out whole sessions rather than trials is essential rather than incidental: only
the within-subject arm can share a session between train and test, so a trial-level split inflates
that arm alone and the bias lands directly on the difference being reported.

The external validation cohort is Allen Visual Behavior two-photon \citep{allenvb2023}: containers
spanning at least 14 days whose experiments are all in VISp within a single 50\,\textmu m depth bin,
leaving 23 mice after per-experiment quality control. Because different animals have no neuron
correspondence and the longitudinal subset is single-plane, the representation that is comparable
across animals is a permutation-invariant summary of the population response, giving a fixed
27-dimensional feature regardless of how many cells were segmented. The decoded contrast is
\texttt{is\_change}, a real image change against a sham catch trial. Appendix~\ref{app:methods}
gives both cohorts in full.
...
\label{sec:protocol}
Every number reported above holds out whole recording sessions. Repeating the identical analysis on
the identical features with a trial-level split inflates the personal decoder's within-block accuracy
by $+0.013$ (lever pull), $+0.010$ (lick), $+0.009$ (reward) and $+0.000$ $\auc$ (tone), while
leaving the pooled decoder untouched. The inflation therefore lands on one side of the difference,
and for lever pull it is the entire effect: a significant-looking $+0.014$ ($p = 0.001$) becomes
$+0.003$ ($p = 0.37$). Any comparison of a within-subject decoder against a cross-subject one is
exposed to this, since only the within-subject arm can share a session between train and test.
...
Observatory). The analysis code and streaming feature pipeline are released; the repository link is
withheld here to preserve anonymity and will be given in the camera-ready version.

\textbf{Limitations.} The four events are not four independent behaviors but four thresholded
channels of one cued lever-pull trial, overlapping to the point where $100\%$ of reward windows also
contain licking, so we cannot always say which behavior the effect belongs to.
...
Finally, the trial-level inflation of Section~\ref{sec:protocol} is a caution rather than a
limitation of this work, because it is a limitation we do not have: every number we report is
measured after holding out whole sessions. The practical consequences are collected in
