...
Aging is the accuracy lost as testing moves from early to late, and the per-mouse asymmetry is
personal aging minus pooled aging, positive when the personal decoder ages more. Significance is a
paired sign-flip permutation test across mice, with cluster-bootstrap confidence intervals.
...
stability could reflect data volume rather than pooling. We test this with a count-matched control (Fig.~\ref{fig:mech}, Appendix~\ref{app:fig}). For each held-out early session $s$ we set the training-set size to
$n$, the number of events in that mouse's remaining early sessions, and compare three decoders:
\emph{self}, trained on those $n$ events; \emph{one}, on $n$ events from a single other mouse; and
\emph{many}, on $n$ events drawn across the other 24 mice. All three are scored on the same held-out
session and late block, so they differ only in whose data they were fit on.
...
training mice confirms this is a secondary effect of around $0.01$ $\auc$: most of the resistance to
...
\textbf{Limitations.} The clearest limitation is what the four events are: not four independent
behaviors but four thresholded channels of one cued lever-pull trial, overlapping to the point where
$100\%$ of reward windows also contain licking, so we cannot always say which behavior the effect
belongs to. A design with temporally separated, count-matched events is the experiment we would run
next.

We also measure at the mesoscale, and the two-photon replication reaches single neurons only at one
depth in one visual area; the analysis covers two weeks in one species. Finally, the trial-level
inflation of Section~\ref{sec:protocol} is a caution rather than a limitation of this work: it is
large enough to manufacture an effect that is not there, and every number we report is measured
after holding out whole sessions (Appendix~\ref{app:practice}).
...
\section{Count-matched control and diversity scaling}
\label{app:fig}
\begin{figure}[H]\centering
\includegraphics[width=0.85\linewidth]{figs/fig_mechanism.pdf}
