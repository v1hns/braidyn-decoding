...
know that conservation across individuals is established, and that pooling works at a fixed time,
every one of those results is measured at a single moment. We offer pooling as a solution to a
different problem: the decay of decoding accuracy over time.

Our core prediction is that a decoder that never saw the subject cannot fit to that subject's
...
inflating the asymmetry. We quantify that inflation, and on lever pull it accounts for the entire
apparent effect. For longitudinal decoding comparisons we recommend session-held-out
...
Allen Common Coordinate Framework regions for 25 mice, each completing $\sim$15 daily operant
...
personal decoder aging more in 73\% of pairs; the per-event breakdown is in Table~\ref{tab:asym} of Appendix~\ref{app:perevent}.
...
stability could reflect data volume rather than pooling. We test this with a count-matched control. For each held-out early session $s$ of each mouse we set the
...
finding is that the personal decoder ages faster. The result does not depend on where the session-inclusion threshold is drawn, giving $+0.022$ to
$+0.024$ across thresholds of three to six sessions per mouse. On this cohort the trial-level split
\emph{lowers} the asymmetry to $+0.014$ ($p = 0.07$): trial shuffling still inflates the personal
decoder by $+0.015$, but scoring one pooled $\auc$ across sessions rather than averaging per-session
values costs $-0.030$ ($p < 10^{-4}$) and dominates.
...
classification is a binary decode of one event against these matched negatives, scored by $\auc$. The four decoders are therefore four independent binary detectors, not a
...
one-sided and lands directly on the quantity of interest; Table~\ref{tab:asym} reports what it
costs per event.
