...
\texttt{braidyn\_nonlinear.json}) carry every reported value together with per-mouse breakdowns
and the 44-parcel ROI table. The independent-cohort replication of Section~\ref{sec:repl} is the
one analysis that is not streamed: it requires the Cardin--Higley parcel-trace archive to be
fetched from figshare first, and re-running it emits \texttt{cardin\_results.json}. All splits,
