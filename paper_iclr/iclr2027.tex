...
cross-animal readout does not inherit. \textbf{(ii) Capacity-invariance} (lever-pull and lick):
nonlinear temporal models (a 1-D CNN and a GRU over the evoked window) raise cross-animal $\auc$ by
...
collapsed mean. (Contribution (i) covers all four events; (ii) is established on lever-pull and
lick.)
...
gap is \emph{stable across a 15-day protocol for every event} and, on the two events we tested,
\emph{does not widen with model capacity}. The one place time is resolvable is \emph{within} an
animal (reward, $\approx0.02$ $\auc$), and notably the cross-animal readout does not inherit it.
...
appears hardest \citep{wicat2026}. The longitudinal analysis (Section~\ref{sec:drift}) covers all
four events, but the capacity analysis (Section~\ref{sec:nl}) is run on lever-pull and lick only,
so whether reward and tone are equally capacity-invariant is untested. Our conclusions rest on
LOMO-vs-within and across-vs-same-block \emph{comparisons} rather than absolute $\auc$, and they
hold for both the linear and the nonlinear decoders. The residual cross-animal gap is small but non-zero: we
...
$0.03$--$0.04$ $\auc$ of the within-animal ceiling, that this small cross-subject gap is stable
across a two-week protocol for every event, and that it does not widen with decoder capacity on the
two events we tested, with an independent-cohort replication. We release code and a streamed-feature pipeline that reproduces every number without
