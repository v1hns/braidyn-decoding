...
$p = 2 \times 10^{-5}$; Section~\ref{sec:asym}).
...
The early/late split is coarse, so we also check that the effect does not depend on the block
boundaries. For each held-out mouse we regress decoding $\auc$ on the test-session day (restricted to
days after the training block, so the personal decoder is never tested on its training data) and
compare the per-mouse day-slope of the personal and pooled decoders. Across all 100 mouse--target
pairs, the personal decoder's slope is less favorable than the pooled decoder's by $0.0016$ $\auc$
per day ($p = 0.00025$; the personal decoder is steeper in 70\% of pairs). This continuous test is
noisier per event (significant for lick and reward, marginal for lever pull, null for tone), but its
pooled direction matches the block result: the personal decoder's day-to-day trajectory decays
faster than the pooled decoder's.
