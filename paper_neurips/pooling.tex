...
own decoder ages quicker in 76 of 100 mouse--target pairs), and the effect holds for all four events
...
for lick, $+0.033$ for reward, and $+0.023$ for tone. Each is significant at $p < 0.005$. Across all
100 mouse--target pairs, the asymmetry is $+0.024$ $\auc$ ($p = 2 \times 10^{-5}$), with the personal
decoder aging more in 76\% of pairs. The same direction holds for lever pull, where both decoders
...
ahead: on day 1 it leads the pooled decoder by about $0.04$ $\auc$, as expected for a decoder trained
on the same animal. That lead then erodes. Across the protocol the personal decoder declines while
the pooled decoder holds flat, and the gap between them narrows by $0.0014$ $\auc$ per day, pooled
over all 100 mouse--target pairs ($p = 5 \times 10^{-5}$; the gap narrows in 64\% of pairs). The
trend is significant for lick and reward, where the personal decoder loses accuracy over days; for
lever pull and tone both decoders improve with practice, so the gap stays flat
(Fig.~\ref{fig:decay}). By day 15 the personal decoder's early advantage has fallen to about
$0.01$ $\auc$.
...
decoder starts ahead and declines; the pooled decoder holds. Their gap narrows for lick and reward,
where the personal decoder loses accuracy over days, and stays flat for lever pull and tone, where
both decoders improve with practice. The per-event gap slope and its $p$ (paired sign-flip over
mice) are shown.}
