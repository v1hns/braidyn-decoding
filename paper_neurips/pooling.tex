...
Population pooling is thus a drift-mitigation strategy in its own right: the early-week cost of
...
overwhelmingly linear \citep{melbaum2022,peterson2021}. All conditions below use this same readout, so the only
variable across conditions is which data the decoder was trained on.
...
components of drift are individual-specific. Finally, we report only linear readouts here; whether a
higher-capacity decoder changes the picture is a question our protocol makes harder rather than
easier to answer, since such a model has more capacity to exploit exactly the within-session
structure Section~\ref{sec:protocol} shows must be excluded.
