...
\begin{abstract}
A decoder fit to one animal's cortex loses accuracy on that animal over days, and the usual remedy is
to recalibrate it on fresh data from the same subject. We report a different kind of robustness: a
decoder built from \emph{other} individuals ages more slowly than the subject's own. On the open
BraiDyn-BC cohort (25 mice, 44 Allen-atlas cortical parcels, a $\sim$15-day operant protocol) we
decode four task events and measure, per mouse, the $\auc$ lost over two weeks by its own early-week
decoder versus one pooled over the other 24 mice. The personal decoder ages more by $+0.017$ $\auc$
($p = 5 \times 10^{-5}$, 73 of 100 mouse--target pairs). The size of the effect varies across the four
targets, but they are chained within a trial -- every reward onset has a lick within $0.12$\,s -- and
differ tenfold in sample size, so we report that variation without attributing it to the behaviors.
At matched training-event count a decoder from a single other mouse still ages less, so this is not a
data-volume effect but a consequence of not overfitting the subject's drift-prone idiosyncrasies; it
survives a 1-D CNN and a GRU as well. One caveat is load-bearing:
trials within a session are not independent, so splitting at the trial level inflates the personal
decoder alone, by up to $0.013$ $\auc$ -- enough to manufacture the entire lever-pull effect. All
estimates here hold out whole sessions. Code and a streamed-feature pipeline are released.
\end{abstract}
...
\item \textbf{Pooling resists drift.} Across two weeks a population-pooled decoder loses less accuracy
than a mouse's own (paired asymmetry $+0.017$ $\auc$, $p = 5 \times 10^{-5}$;
Section~\ref{sec:asym}). The effect varies in size across the four decoded targets, but those targets
are chained within a trial and differ tenfold in event count, so we treat the pooled estimate as the
result and the per-target ranking as uninterpreted (Section~\ref{sec:targets}).
...
other three events, and Section~\ref{sec:targets} reports how often they contain one. Every
...
The effect is not uniform across the four targets: it is largest for reward ($+0.029$,
$p = 0.0005$) and lick ($+0.020$, $p = 0.002$), marginal for tone ($+0.015$, $p = 0.08$), and absent
for lever pull ($+0.003$, $p = 0.37$). We report this variation but deliberately do not build an
interpretation on it. As Section~\ref{sec:targets} shows, the four targets are neither temporally
separable nor matched in sample size, so the ranking across them cannot be read as a statement about
which \emph{behaviors} carry the effect. The pooled estimate is the claim we defend; the per-event
column is data.
...
\subsection{What the four targets are, and why we do not compare them}
\label{sec:targets}
The four decoded events come from a single cued lever-pull trial structure, so they are chained in
time rather than independent. Measuring onset times directly on eight sessions from eight mice: every
reward onset has a lever-pull onset a median of $0.083$\,s away and a lick onset $0.117$\,s away, and
$100\%$ and $98\%$ respectively fall inside the $[-0.5, +1.0]$\,s window from which a feature vector
is built. Taken over all four targets, the fraction of positive windows containing at least one other
event is $100\%$ (reward), $84\%$ (lick), $84\%$ (tone) and $68\%$ (lever pull). The reward decoder
is therefore never reading reward in isolation; its input window always contains licking and lever
movement as well.

The negatives are also not event-free. Because they are screened only against the target's own
onsets, they contain another event $33$--$57\%$ of the time ($56\%$ for reward, $57\%$ for tone).
What these $\auc$s measure is thus closer to ``this task moment versus another moment, which is often
also a task moment'' than to event versus rest, and the absolute values should be read accordingly.

The targets also differ in sample size by more than an order of magnitude: a median of $56$ reward
onsets per session against $107$ (tone), $530$ (lick) and $652$ (lever pull). The per-event asymmetry
runs roughly opposite to that count (Spearman $\rho \approx -0.8$ across the four targets, which with
$n = 4$ is suggestive at most). A decoder fit to fewer events has more idiosyncrasy available to
overfit, and therefore more for pooling to avoid, which would produce exactly this ordering without
any of the behaviors differing.

We cannot separate these explanations with four targets, and we do not try. The consequence for the
rest of the paper is deliberate: the pooled estimate over all 100 mouse--target pairs is what we
defend, and the per-event breakdown in Table~\ref{tab:asym} is reported so the variation is visible,
not as four independent findings. Where later sections note that an analysis selects reward and lick
(Sections~\ref{sec:decay}, \ref{sec:matched}, \ref{sec:nl}), that is a consistency check on the same
targets, not evidence from independent behaviors.
...
\textbf{Limitations.} The clearest limitation is what our four targets are. They are not four
independent behaviors but four thresholded channels of one cued lever-pull trial, overlapping in time
to the point where every reward window also contains licking, and differing tenfold in event count
(Section~\ref{sec:targets}). We therefore cannot say which behavior the effect belongs to, nor
exclude that the per-target ranking simply tracks how much data each decoder had to overfit. A design
with temporally separated, count-matched events would settle both, and is the experiment we would run
next. The pooled estimate over all 100 mouse--target pairs should be read as an average over four
correlated readouts of one task, not as a uniform property of cortex. We measure at the mesoscale, using
